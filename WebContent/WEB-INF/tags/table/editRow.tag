...
<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="edit()" title="编辑"><i class="fa fa-file-text-o"></i> ${label==null?'编辑':label}</button>
                        </button>
...
$(document).ready(function() {
	...
	function edit(){
		  var message = '${title}' == "请至少选择一条数据!" ? "":'请选择一条${title}数据进行编辑';
		  var messageOnly = '${title}' == "只能选择一条数据!" ? "":'只能选择一条${title}数据进行编辑';
		  var size = $("#${id} tbody tr td input.i-checks:checked").size();
		  if(size == 0 ){
				top.layer.alert(message, {icon: 0, title:'警告'});
				return;
			  }

		  if(size > 1 ){
				top.layer.alert(messageOnly, {icon: 0, title:'警告'});
				return;
			  }
