	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			validateForm = $("#inputForm").validate({
				rules: {
					loginName: {remote: "${ctx}/sys/user/checkLoginName?uid=${parents.id}&${parents.loginName}"},
					idNo: {remote: "${ctx}/sys/user/checkLoginName?uid=${parents.id}&${parents.idNo}"},
					email: {remote: "${ctx}/sys/user/checkLoginName?uid=${parents.id}&${parents.email}"},
					mobile: {remote: "${ctx}/sys/user/checkLoginName?uid=${parents.id}&${parents.mobile}"}
				},
				messages: {
			...
			* 初始化扩展字段
			*/
			extendField.initExtendFieldTable("PARENTS", $("#id").val(), '${ctx}/fieldconfig/userExtendInfo/usablefieldList',"extendInfoDiv",2);
		});
	</script>
</head>
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="parents" action="${ctx}/account/parents/save" method="post" class="form-horizontal">
				...
				<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
				   <tbody>
				      <tr>
                         ...
                         <td><input id="oldLoginName" name="oldLoginName" type="hidden" value="${parents.loginName}">
                       ...
                       <c:if test="${empty parents.id}"> <tr>
                         <td class="active"><label class="pull-right"><c:if test="${empty parents.id}"><font color="red">*</font></c:if>密码:</label></td>
                         <td><input id="newPassword" name="newPassword" type="password" value="" maxlength="50" minlength="${pwsmin }" 
                         	class="form-control ${empty parents.id?'required':''} ${complex ? 'specialChar beginChar pwsChar':''}" rangelength="[${pwsmin },20]"/>
                           </td>
                         <td class="active"><label class="pull-right"><c:if test="${empty parents.id}"><font color="red">*</font></c:if>确认密码:</label></td>
                         <td><input id="confirmNewPassword" name="confirmNewPassword" type="password"  
                         	class="form-control ${empty parents.id?'required':''} ${complex ? 'specialChar beginChar pwsChar':''}" rangelength="[${pwsmin },20]"
                         	value="" maxlength="50" minlength="${pwsmin }" equalTo="#newPassword"/></td>
                      </tr></c:if>
                      <tr>
                      ...
                      <tr>
                         ...
                         <td class="width-35">
                         <input type="hidden" name="company.id" value="${parents.company.id}"/>
                         <form:input path="company.name" htmlEscape="false" maxlength="100" class="form-control" readonly="true"/></td>
                         
                         <td class="width-15" class="active"><label class="pull-right"><font color="red">*</font>学生:</label></td>
                         <td class="width-35">
                            <input id="studentIds" name="studentIds" class="form-control" type="hidden" value="${parents.studentIds}">
    <div class="input-group">
        <input id="studentNames" name="studentNames" readonly="readonly" type="text" value="${parents.studentNames}" class="form-control required" style=""  aria-required="true">
             <span class="input-group-btn">
                 <button type="button" id="studentButton" class="btn   btn-primary  "><i class="fa fa-search"></i>
                 </button> 
             </span>
            
    </div>
     <label id="studentName-error" class="error" for="studentNames" style="display:none"></label>
     <script type="text/javascript">
    $("#studentButton, #studentNames").click(function(){
        // 是否限制选择，如果限制，设置为disabled
        if ($("#studentButton").hasClass("disabled")){
            return true;
        }
        var url="${ctx}/account/common/officeStudentTree?id=";
        if($("#id").val()){
            url+=$("#id").val();
        }
        // 正常打开 
        top.layer.open({
            ...
            btn: ['确定', '关闭']
               ,yes: function(index, layero){ //或者使用btn1
                        ...
                        //console.log(ids);
                        //console.log(names);
                        $("#studentIds").val(ids.join(",").replace(/u_/ig,""));
                        $("#studentNames").val(names.join(","));
                        $("#studentNames").focus();
        ...
        cancel: function(index){ //或者使用btn2
                   //按钮【按钮二】的回调
                      ...
                      <tr>
                         ...
                         <td>
                         <input type="hidden" id="roleType" name="roleType" value="${parents.roleType }"/>
                         ...
                         <td>
                         <input id="roleIds" name="roleIds" class="form-control" type="hidden" value="${parents.roleIds}">
    <div class="input-group">
        <input id="roleNames" name="roleNames" readonly="readonly" type="text" value="${parents.roleNames}" class="form-control valid" style=""  aria-invalid="false">
             <span class="input-group-btn">
        ...
        cancel: function(index){ //或者使用btn2
                   //按钮【按钮二】的回调
                      ...
                      <tr>
                         ...
                         <td>
                         <input id="groupIds" name="groupIds" class="form-control" type="hidden" value="${parents.groupIds}">
    <div class="input-group">
        <input id="groupNames" name="groupNames" readonly="readonly" type="text" value="${parents.groupNames}" class="form-control valid" style=""  aria-invalid="false">
             <span class="input-group-btn">
    ...
    $("#groupButton, #groupNames").click(function(){
        ...
        cancel: function(index){ //或者使用btn2
                   //按钮【按钮二】的回调
                         ...
                         <td><input type="hidden" id="origin" name="origin" value="${parents.origin }"/>
