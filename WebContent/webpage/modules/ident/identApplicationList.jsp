		...
		<div class="ibox">
			...
			<div class="ibox-content">
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="application"
							action="${ctx}/ident/application/list" method="post" class="form-inline">
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="pull-left">
							<shiro:hasPermission name="ident:application:add">
								<table:addRow url="${ctx}/ident/application/form" title="应用" width="800px"
									height="500px"></table:addRow>
								<!-- 增加按钮 -->
							</shiro:hasPermission>
							<shiro:hasPermission name="ident:application:edit">
								<table:editRow url="${ctx}/ident/application/form" id="contentTable"
									title="应用" width="800px" height="500px"></table:editRow>
								<!-- 编辑按钮 -->
							</shiro:hasPermission>
							<shiro:hasPermission name="ident:application:del">
								<table:delRow url="${ctx}/ident/application/deleteByLogic" id="contentTable"></table:delRow>
								<!-- 删除按钮 -->
							</shiro:hasPermission>
					...
					<tbody>
						<c:forEach items="${page.list}" var="application">
							<tr>
								...
								<td>
									<c:choose>  
				                        <c:when test="${application.status eq 1 }">
								...
								<td class="width-15">
									<c:if test="${application.accessWay eq 1 }">
										<shiro:hasPermission name="ident:application:authority">
											<a href="#"
												onclick="openDialogView('查看权限', '${ctx}/ident/application/authority?id=${application.id}','800px', '500px')"
												title="查看权限"
												class="btn btn-primary btn-xs btn-circle"><i
												class="fa fa-file"></i></a>
										</shiro:hasPermission>
										<shiro:hasPermission name="ident:application:manager">
											<a href="#"
												onclick="openDialog('指定管理员', '${ctx}/ident/application/manager?id=${application.id}','800px', '500px')"
												title="指定管理员"
												class="btn btn-info btn-xs btn-circle"><i
												class="fa fa-user"></i></a>
										</shiro:hasPermission>
									</c:if>
<%-- 									<shiro:hasPermission name="ident:application:user"> --%>
<!-- 										<a href="#" -->
<%-- 											onclick="openDialog('管理成员', '${ctx}/ident/application/user?id=${application.id}','800px', '500px')" --%>
<!-- 											title="管理成员" -->
<!-- 											class="btn btn-warning btn-xs btn-circle"><i -->
<!-- 											class="fa fa-users"></i></a> -->
<%-- 									</shiro:hasPermission> --%>
									<shiro:hasPermission name="ident:application:enable">
										<c:if test="${application.status eq 1}">
											<a href="${ctx}/ident/application/disable?id=${application.id}"
											onclick="return confirmx('确认要禁用该应用吗？', this.href)"
											title="禁用"
											class="btn btn-danger btn-xs btn-circle"><i
											class="fa fa-lock"></i></a>
										</c:if>
									</shiro:hasPermission>
									<shiro:hasPermission name="ident:application:disable">
										<c:if test="${application.status eq 2}">
											<a href="${ctx}/ident/application/enable?id=${application.id}"
											onclick="return confirmx('确认要启用该应用吗？', this.href)"
											title="启用"
											class="btn btn-danger btn-xs btn-circle"><i
