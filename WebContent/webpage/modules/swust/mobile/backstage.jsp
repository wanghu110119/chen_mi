	...
	<div class="row">
						...
						<li>
							...
							<form id="jvForm" method="post" enctype="multipart/form-data">
								<a href="javascript:void(0);" class="file"> 选择背景 <input
									type="file" name="photo" id="choose-file"
									onchange="uploadPic()">
								</a>
							</form>
						</li>
		...
		<script type="text/javascript">
                                        function uploadPic() {
                                            ...
                                            $.ajax({  
                                            	          url: "mobile/swust/system/insertUserPhoto" ,  
                                            	          type: 'POST',  
                                            	          data: formData,  
                                            	          async: false,  
                                            	          cache: false,  
				...
				<div class="col-xs-12 col-sm-5 less-width">
					...
					<form>
								...
								<c:choose>
									<c:when test="${requestScope.redio.id eq '1' }">
										<input type="radio" name="type-time"
											class="type-time first-choice-radio" checked="checked"  onclick="changeRedio(1)"/>
									</c:when>
									<c:otherwise>
										<input type="radio" name="type-time"
						...
						<div class="form-group col-sm-12">
							<label style="text-align: left !important; margin-left: 7.5%" id="forDays">预约天数（天）：</label>
							<div class="input-group date form_datetime required">
								<c:choose>
									<c:when test="${requestScope.redio.id eq '2' }">
										<input type="radio" name="type-time"
											class="type-time second-choice-radio" checked="checked" onclick="changeRedio(2)"/>
									</c:when>
									<c:otherwise>
										<input type="radio" name="type-time"
											class="type-time second-choice-radio" onclick="changeRedio(2)" />
									</c:otherwise>
								</c:choose>
<!-- 								<input id="beginDate" type='text' -->
<!-- 									class="form-control parameter required md second-choice" onclick="changeDate()" -->
<!-- 									style="margin-left: 7.5%" readonly  -->
<%-- 									value="${requestScope.beginDate } -- ${requestScope.endDate }"> --%>
								<select class="form-control md second-choice" id="days" style="margin-left: 7.5%;width:32.5%;" id="choose-days" onchange="changeDate()"></select>
							</div>
						</div>
						<!--                                        <div class="form-group"> -->
						<!--                                         <label for="choose-days">预约天数：</label> -->
						<!--                                   		 <div class="input-group days" > -->
						<%-- 										<input id="changeDay" type='number'  min="1" onblur="ChangeDay()" value="${requestScope.orderDay }"/> --%>
						<!-- 										</div> -->
						<!--                                     </div> -->
									...
									<c:forEach items="${carList }" var="list" begin="0" end="0">
										<tr id="tr_${list.id }">
											...
											<td>起价 <input type="number" min="0"
												...
												onchange="changeMoney('${list.id }')">元。
											</td>
											<td><input type="number" min="0"
												value="${list.moneyMonth }"
												onchange="changeMoney('${list.id }')" name="moneyMonth">元/辆/月
											</td>
										</tr>
									</c:forEach>  
									<!-- 大型汽车 -->
									<c:forEach items="${carList }" var="list" begin="1" end="2">
										<tr>
											<td>${list.remarks }<input type="hidden" name="id"
												value="${list.id }" /> <input type="hidden" name="remarks"
												value="${list.remarks }" />
											...
											<td><input type="number" min="0"
												...
												onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)"
												onblur="this.v();" name="maxMoney"
												onchange="changeMoney('${list.id }')">元。</td>
											<td><input type="number" min="0"
												value="${list.moneyMonth }"
												onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)"
												onblur="this.v();" name="moneyMonth"
												onchange="changeMoney('${list.id }')">元/辆/月</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
		...
		$('.first-choice').attr('disabled',true);
              ...
              $(".modify").click(function () {
            	  ...
            	  function changeRedio(id) {
					$.ajax({
						url:"mobile/swust/system/changeRedio",
						type:"post",
						data:{
							"id" : id,
						},
				...
				function changeMessage() {
            	  $.ajax({  
        	          url: "mobile/swust/system/changeMessage" ,  
        	          type: 'post',  
        	          data: {
              ...
              function ChangeDay(){
            	  $.ajax({  
        	          url: "mobile/swust/system/changeDay" ,  
        	          type: 'post',  
        	          data: {
              ...
              function changeTime(beginTime, endTime) {
            	  $.ajax({  
        	          url: "mobile/swust/system/changeTime" ,  
        	          type: 'post',  
        	          data: {
						"beginTime" : beginTime,
						"endTime":endTime,
              ...
              $("#choose-file").change(function () {
            	    ...
            	    if (fileObj && fileObj.files && fileObj.files[0]) {
            	      ...
            	      $('<li><img src="${dataURL}" class="img-responsive"/></li>').insertBefore(".back-right li:last-child");
            	      systemcontroler();
            	    }
            	  });
            function changeDate() {
...
//        	  });
              ...
              function changepicture(id) {
              	 $.ajax({  
            	          url: "mobile/swust/system/changepicture" ,  
            	          type: 'POST',  
            	          data: "id="+id,
            	       success: function (data) {  
                 },   
              ...
              $(".back-right").on("click","img",function () {
				...
				function deletepicture(id){
					 $.ajax({  
           	          url: "mobile/swust/system/deletepicture" ,  
           	          type: 'POST',  
           	          data: "id="+id,
           	       success: function (data) {  
                },   
