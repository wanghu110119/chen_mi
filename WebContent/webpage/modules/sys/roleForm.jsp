...
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="role" autocomplete="off" action="${ctx}/sys/role/save" method="post" class="form-horizontal" >
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      <tr>
		         <td class="width-15 active"><label class="pull-right"><font color="red">*</font>上级角色:</label></td>
		         <td class="width-35"> <sys:treeselect id="office" name="parent.id" value="${role.parent.id}" labelName="parent.name" labelValue="${role.parent.name}"
                    title="角色" url="/sys/role/treeData" extId="${role.id}"  cssClass="form-control required" /></td>
		         ...
		         <td class="width-35"><input id="oldName" name="oldName" type="hidden" value="${role.name}">
					<form:input path="name" htmlEscape="false" maxlength="50" class="form-control specialChar required"/></td>
		      </tr>
		      <tr>
		         ...
		         <td class="width-35"><input id="oldEnname" name="oldEnname" type="hidden" value="${role.enname}">
					<form:input path="enname" htmlEscape="false" maxlength="50" class="form-control specialChar required"
					disabled="${isupdate }"/></td>
