...
<script type="text/javascript">
	function refresh() {//刷新
		window.location = "${ctx}/unifiedAuth/identityGroup";
	}
</script>
</head>
<body class="gray-bg">
					...
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="user" action=""
							...
							<div class="form-group">
								<span>用户组名称：</span> <input type="text"
									class=" form-control input-sm" id="searchName" />
				...
				<div id="content" class="row">
					<div id="left" class="leftBox col-sm-1 left-index">
						...
						<ul class="nav nav-tabs" role="tablist" id="left-ul">
						    <li role="presentation" class="active"><a href="#userGroup" aria-controls="role" role="tab" data-toggle="tab">用户组</a></li>
						</ul>
	...
	<script type="text/javascript">
		var setting = {
			...
			callback : {
				onClick : function(event, treeId, treeNode) {
					...
					$('#authContent').attr(
							"src",
							"${ctx}/unifiedAuth/identityGroup/list?id=" + id
		...
		function refreshTree() {
			$.getJSON("${ctx}/unifiedAuth/identityGroup/treeData", function(
					data) {
				...
				if (nodes.length > 0) {
					...
					$('#authContent').attr(
							"src",
							"${ctx}/unifiedAuth/identityGroup/list?id=" + id + "&groupName="
									+ name);
				}
		...
		function search() {
			...
			if (searchName) {	
				$('#authContent').attr(
						"src",
						"${ctx}/unifiedAuth/identityGroup/identityGroupList?searchName="
								+ searchName);
			}
		}
	</script>
