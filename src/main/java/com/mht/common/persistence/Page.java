 ...
 */
package com.mht.common.persistence;
...
public class Page<T> {
	...
	private int begin;//设置遍历缩进开始
	
	private int end;//设置遍历缩进结束
	
	/**
	 * 
	 * @Title: setBeginAndEnd 
	 * @Description: TODO
	 * @param page
	 * @param size
	...
	public String toString() {
		//自定义分页方法
		return pageBottomNew();
		//框架原始分页方法
//	return pageBottom();
	}
	/**
	 * 
	 * @Title: pageBottomNewWZW 
	 * @Description: TODO
	 * @return
	 * @author com.mhout.wzw
	 */
	private String pageBottomNew() {
		StringBuilder sb = new StringBuilder();
		int maxpi = this.getTotalPage();
		int pi = pageNo;
		if (pageNo == first) {// 如果是首页
			sb.append("<button class='no-style'>上一页</button>");
		} else {
			sb.append("<button class='no-style' onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\">上一页</button>");
		}
		if(maxpi<=10){
			for(int i=1;i<maxpi+1;i++){
				String fnvalue = i==pi?"active":"";
				sb = sb.append("<button class=\""+fnvalue+"\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"+i+"</button>");
			}
		}else{
			if(pi-4<=1){
				for(int i=1;i<=10;i++){
				...
				for(int i=maxpi-9;i<maxpi+1;i++){
					String fnvalue = i==pi?"active":"";
					sb = sb.append("<button class=\""+fnvalue+"\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"+i+"</button>");
				}
//				if(pi+5<=maxpi){
//					for(int i=pi-4;i<pi+5;i++){
//						String fnvalue = i==pi?"active":"";
//						sb = sb.append("<button class=\""+fnvalue+"\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"+i+"</button>");
//					}
//				}else{
//					if(pi<=maxpi){
//						for(int i=1;i<=10;i++){
//							int pagei = i+(maxpi-10);
//							String fnvalue = pagei==pi?"active":"";
//							sb = sb.append("<button class=\""+fnvalue+"\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"+pagei+"</button>");
//						}
//					}
//				}
			}else{
		...
		if (pageNo == last) {
			sb.append("<button class='no-style'>下一页</button>");
		} else {
			sb.append("<button class='no-style' onclick=\""+funcName+"("+next+","+pageSize+",'"+funcParam+"');\">下一页</button>");
		}
		return sb.toString();
	}
...
//		sb.append("</div>");
		...
		//如果总数为0.则应该显示0到0
		sb.append("<span class=\"pagination-info\">显示第 "+ (count== 0 ? 0 : startIndex)+" 到第 "+ endIndex +" 条记录，总共 "+count+" 条记录</span>");
		//如果pageSize 是在0-10之间，说明是自定义的页面显示条数，则不显示下拉
		if(pageSize <= 0 || pageSize >= 10){
			sb.append("<span class=\"page-list\">每页显示 <span class=\"btn-group dropup\">");
			sb.append("<button type=\"button\" class=\"btn btn-default btn-sm btn-outline dropdown-toggle\" style=\"width:80px;\" data-toggle=\"dropdown\" aria-expanded=\"false\">");
			sb.append("<span class=\"page-size\">"+pageSize+"</span> <span class=\"caret\"></span>");
			sb.append("</button>");
			sb.append("<ul class=\"dropdown-menu\" role=\"menu\" style=\"min-width:100%;\">");
