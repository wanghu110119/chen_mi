...
public interface OfficeDao extends TreeDao<Office> {
     ...
     * @author com.mhout.sx
     */
    @Delete("delete from sys_office_post where office_id=#{1}")
     ...
     * @author com.mhout.sx
     */
    @Insert("insert into sys_office_post(office_id,post_id) VALUES(#{0},#{1})")
     ...
     * @author com.mhout.sx
     */
    @Select("select p.id,p.name from sys_post p,sys_office_post op where op.office_id=#{1} and p.del_flag='0' and p.id=op.post_id")
     ...
     * @author com.mhout.sx
     */
    @Delete("delete from sys_user_office where user_id=#{0}")
     ...
     * @author com.mhout.sx
     */
    @Insert("insert into sys_user_office(user_id,office_id) VALUES(#{0},#{1})")
     ...
     * @author com.mhout.sx
     */
    @Select("select id,name,parent_ids,grade from sys_office o,sys_user_office uo where uo.user_id=#{0} and uo.office_id=o.id and o.del_flag='0'")
     ...
     * @author com.mhout.sx
     */
    @Select("select id,name from sys_office where name=#{0}")
     ...
     * @author com.mhout.sx
     */
    @Select("select id,name,parent_id \"parent.id\" from sys_office where parent_id=#{0} and del_flag='0'")
     ...
     * @author com.mhout.sx
     */
    @Select("select p.id,p.name,up.user_id otherId from sys_post p inner join sys_office_post op on (op.office_id=#{0} and op.post_id=p.id) "
            + "left join sys_user_post up on (up.user_id=#{1} and up.post_id=p.id) where  p.del_flag='0' ")
     ...
     * @author com.mhout.sx
     */
    @Select("select id from sys_office where name=#{0} and del_flag='0'")
     ...
     * @author com.mhout.sx
     */
    @Select("select id,name,parent_id \"parent.id\" from sys_office where parent_id=#{0} and (parent_id='0' or type=#{1}) and del_flag='0'")
     ...
     * @author com.mhout.sx
     */
    @Select("select u.id,u.name,u.no,ps.student_id otherId from sys_user u inner join sys_user_office uo on (uo.office_id=#{0} and uo.user_id=u.id and u.role_type=#{2}) "
            + "left join sys_parents_student ps on (ps.parents_id=#{1} and ps.student_id=u.id) where  u.del_flag='0' ")
     ...
     * @author com.mhout.sx
     */
    @Select("select u.id,u.name from sys_user u inner join sys_user_office uo on (uo.office_id=#{0} and uo.user_id=u.id and u.role_type=#{1}) where u.del_flag='0'")
	...
	@Select("select max(cast(code as decimal)) from sys_office")
	public String findOfficeMaxCode();
	
	public void updateState(@Param("id")String id, @Param("useable")String useable);
	
	@Select("select id,login_name from sys_user where office_id = #{0}")
