...
import com.mht.modules.sys.entity.SystemConfig;
import com.mht.modules.sys.entity.User;
import com.mht.modules.sys.service.LogService;
import com.mht.modules.sys.service.SystemConfigService;
import com.mht.modules.sys.service.SystemService;
import com.mht.modules.sys.utils.UserUtils;
import com.mht.modules.tools.utils.TwoDimensionCode;
...
public class UserController extends BaseController {
    ...
    public String posts(String officeId, String userId, Model model) {
        // 获取post列表
        List<Post> list = this.postDao.getAllPostForUserOffice(officeId, userId);
        model.addAttribute("posts", list);
        return "modules/sys/userPosts";
    }
    ...
    public String form(User user, Model model) {
        ...
        if (!StringUtils.isBlank(user.getId())) {
            List<Post> posts = postDao.findPostByUser(user.getId());
            ...
            user.setPostIds(String.join(",", ids));
            user.setPostNames(String.join(",", names));
        }
    ...
    public String save(User user, HttpServletRequest request, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (!"true".equals(checkLoginName(user,null))) {
        ...
        // 保存用户信息
        systemService.saveUser(user, request.getParameterMap(), GroupRole.STUDENT);
        // 清除当前用户缓存
    ...
    public String importFile(MultipartFile file, RedirectAttributes redirectAttributes) {
        ...
        try {
            ...
            for (User user : list) {
                try {
                    if ("true".equals(checkLoginName(user,null))) {
                        user.setPassword(CommonService.encryption("123456"));
    ...
    public String checkLoginName(User user, String uid) {
    	user.setId(uid);
    ...
    public String infoEdit(User user, boolean __ajax, HttpServletResponse response, Model model,
        ...
        if (StringUtils.isNotBlank(user.getName())) {
            ...
            if (user.getNation() != null)
                currentUser.setGender(user.getNation());
            systemService.updateUserInfo(currentUser);
            addMessage(redirectAttributes, "保存成功");
            return "redirect:" + adminPath + "/sys/user/info?repage";
        }
        if (currentUser.getCompany() == null || currentUser.getCompany().getId() == null) {
            currentUser.setCompany(UserUtils.getUser().getCompany());
        }
        model.addAttribute("user", this.systemService.editUser(currentUser));
        model.addAttribute("list", this.systemService.findAuthUser(currentUser));
        return "modules/sys/userInfoEdit";
    }
    ...
    public String userLogList(Log log, HttpServletRequest request, HttpServletResponse response, Model model) {
        User user = UserUtils.getUser();
        List<String> types = new ArrayList<String>();
        types.add(Log.TYPE_LOGIN);
        types.add(Log.TYPE_LOGOUT);
        log.setTypes(types);
